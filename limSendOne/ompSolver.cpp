...
void ompBasis(Grid& grid, Matrix& mat, double* basis, Options opt){
  ...
  double update_sum[NF]; for (int i = 0; i<NF; i++) update_sum[i] = 0;
  // Storage for the update values
  double basis_update[NB]; for (int i = 0; i<NB; i++) basis_update[i] = 0;
  ...
  for (int iter = 0; iter < opt.maxIter; iter++){
      ...
      for (int coarseIx = 0; coarseIx < NC; coarseIx++) {
        ...
        for (int j = start; j < stop; j++) {
  		...
  		for (int i = 0; i < NB; i++) {
  			...
  			if(type == 0) basis_update[i] += opt.omega*cache[i];
  			else if (type == 2) {
  	...
  	/* **************STEP 3: Modification TWO ********************* */
  		...
  		for (int i = 0; i < NB; i++) {
  			...
  			if (check_convergence && fabs(basis_update[i]) > opt.tolerance && grid.celltypes[i] == 0) done = false;
  			basis_update[i] = 0;
  		}

  		/* ***************STEP 5: Renormalize************************ */
  		if((iter + 1) % opt.reNorm == 0 && iter > 1) renormalize(grid, mat, basis);
