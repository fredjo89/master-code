...
void makeTypeTwo_local(Grid& grid, Node& node, TypeTwo& tt){
    ...
    for (int i = 0; i < grid.n_sup; i++){
        if (grid.celltypes[i] == 2){
            ...
            if ( TTcells[grid.support[i] - min] == 0 ){
                tt.n_cNumbers++;
            }
            TTcells[grid.support[i] - min]++;
            if (TTcells[grid.support[i] - min] > maxTT){
                maxTT = TTcells[grid.support[i] - min];
            }
            tt.n_cIndices++;
        }
    }
    ...
    for (int i = 0; i < node.n_sendCells; i++){
        tt.cNumbers[i] = node.sendCells[i];
    }
...
void makeTypeTwo_serial(Grid& grid, TypeTwo& tt){
    ...
    for (int i = 0; i < grid.n_sup; i++){
        if (grid.celltypes[i] == 2){
            temp1[2*tt.n_cIndices] = grid.support[i];
            temp1[2*tt.n_cIndices+1] = i;
            if (TTcells[grid.support[i]]==0){
                tt.n_cNumbers++;
            }
            TTcells[grid.support[i]]++;
            if (TTcells[grid.support[i]]>maxTT){
                maxTT = TTcells[grid.support[i]];
            }
            tt.n_cIndices++;
        }
    }
    ...
    tt.cNumbers = new int[tt.n_cNumbers];
    tt.cIndices = new int[tt.n_cIndices];
    tt.cIndices_Offsets = new int[tt.n_cNumbers+1];
    tt.cIndices_Offsets[0] = 0;
...
// ********************************************************************************************** //
void localSum(Grid& grid, TypeTwo& tt){
    for (int i = 0; i<tt.n_cNumbers; i++){
        tt.sums[i] = 0;
    }
...
// ********************************************************************************************** //
void TTnormalize(Grid& grid, TypeTwo& tt){
    for (int i = 0; i < tt.n_cNumbers; i++){
        for (int j = tt.cIndices_Offsets[i]; j < tt.cIndices_Offsets[i+1]; j++ ){
