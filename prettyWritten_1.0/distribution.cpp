...
void makeH(Grid& grid, H_data& H){
    ...
    int* temp3 = new int[ grid.N*H.TT_max ];
    int* temp4 = new int[grid.N]();

    for (int i = 0; i < grid.M; i++){
        for (int j = temp2_offsets[i]; j < temp2_offsets[i+1]; j++){
            temp3[ temp2[j]*H.TT_max + temp4[temp2[j]] ] = i;
            temp4[temp2[j]]++;
        }
    }
    ...
    k = 0;
    for (int i = 0; i < grid.N; i++){
        for (int j = H.TT_max*i; j < H.TT_max*i +temp4[i]; j++){
...
void setupNodes(Graph& graph, Node& node, int& RANK){
    ...
    if (RANK==0){
        ...
        for (int i = 0; i<node.n_edges; i++){
            node.edges[i] = graph.edges[i];
        }
        for (int i = 0; i<node.n_sendCells; i++){
            node.sendCells[i] = graph.sendCells[i];
        }
    }
    else{
        int* recvBuff = new int[5];
        MPI_Status* status = new MPI_Status[2]();
        MPI_Recv(recvBuff, 5, MPI_INT, 0, 666, MPI_COMM_WORLD, &status[0] );

        node.number = RANK;
        node.n_edges = recvBuff[0];
        node.weight = recvBuff[1];
        node.maxRow = recvBuff[2];
        node.n_basis = recvBuff[3];
        node.n_sendCells = recvBuff[4];
        node.edges = new int[ node.n_edges ];
        node.sendCells = new int[ node.n_sendCells ];
