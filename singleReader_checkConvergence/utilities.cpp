...
void writeBasis(Grid& g_grid){
    ofstream outfile("/home/shomeb/f/fredjoha/Desktop/master-code/matlab/plots/MPI_converged.txt");
    if (!outfile.is_open()) {cout << "Error opening file(s)\n";return;}
    ...
    for (int i = 0; i < n_sup; i++){
        //outfile<<basis[i]<<" ";
        outfile << std::fixed << setprecision(16) << basis[i]<<" ";
    }
    outfile.close();
...
void computeError(Grid& grid, double* solution, double* infNorm, double* twoNorm, int it){
    ...
    double* error = new double[n_sup];

    double inf = 0;
    double two = 0;

    for (int i = 0 ; i < n_sup; i++){
        error[i] = abs( basis[i]-solution[i] );
        if (error[i] > inf ){
            inf = error[i];
        }
    }
...
void writeErrors(double* infNorm, double* twoNorm, int it){
    ofstream outfile("/home/shomeb/f/fredjoha/Desktop/master-code/matlab/plots/MPI_infErrors.txt");
    if (!outfile.is_open()) {cout << "Error opening file(s)\n";return;}

    for (int i = 0; i < it; i++){
        //outfile<<infNorm[i]<<" ";
        outfile << std::fixed << setprecision(16) << infNorm[i]<<" ";
    }
    outfile.close();


    ofstream outfile2("/home/shomeb/f/fredjoha/Desktop/master-code/matlab/plots/MPI_twoErrors.txt");
    if (!outfile2.is_open()) {cout << "Error opening file(s)\n";return;}

    for (int i = 0; i < it; i++){
        //outfile2<<twoNorm[i]<<" ";
        outfile2 << std::fixed << setprecision(16) << twoNorm[i]<<" ";
    }
    outfile2.close();
