...
void orderGS_Basis(Grid& grid, Matrix& mat, double* basis, Options opt){
  ...
  double prevBasis[n_basis];

  // GS iteration
  for (int i = 0; i<opt.maxIter; i++){
    bool done = true;

    for (int k = 0; k<n_basis; k++) prevBasis[k] = orderBasis[k];

    for (int k = 0; k<grid.M; k++){
      basisArray[k].jacobiProduct(opt.tolerance);
    }



    renormalize(grid, mat, orderBasis);

    done = true;
    for (int k = 0; k<n_basis; k++){
      if (fabs(prevBasis[k] - orderBasis[k]) > opt.tolerance) done = false;
    }


    if (done){
      cout<<"Ordered GS converged after "<< i+1 <<" iterations."<<endl;
      break;
    }
    if (i+1 == opt.maxIter){
      cout<<"Ordered GS did not converge after "<<i+1<<" iterations."<<endl;
    }
