...
/*
    for (int i = 0; i < mat.n_basis; i++){
        int k = grid.support[i];
        for (int j = 0; j < mat.maxRow; j++){
            mat.loc_index[i * mat.maxRow + j] = mat.j_index[k * mat.maxRow +j];
            mat.loc_conn[i * mat.maxRow + j] = mat.conn[k * mat.maxRow +j];
        }
    }
*/
    ...
    for (int it = 0; it < mat.n_basis*mat.maxRow; it++){
        if (j==mat.maxRow){
            j = 0;  i++;    k = grid.support[i]*mat.maxRow;
        }
