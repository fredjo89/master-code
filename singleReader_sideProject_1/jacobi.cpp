...
void makeMapping(Grid& grid, Matrix& mat){
    ...
    for (int i = 0; i<grid.M; i++){
        ...
        for (int j=start*mat.maxRow; j<end*mat.maxRow; j++){
            index = mat.sup_index[j];
            if (index!=-1){
                if (index>=min && index<=max)   mat.sup_index[j] = myMap[index - min];
                else                            mat.sup_index[j] = -2;
            }
        }
        delete[] myMap;
    }
...
void jacobi(Grid& grid, Matrix& mat, double& omega){
    ...
    for (int i = 0; i < grid.n_sup; i++){
        ...
        it = i*mat.maxRow;
        for (j = 0; j < mat.maxRow; j++){
            index = mat.sup_index[it+j];
            if (index==-1) break;
            if (index!=-2){
                //grid.updates[i] += mat.sup_coef[it+j]*grid.basis[index];
                grid.updates[i] += mat.mat_coef[mat.map[i]*mat.maxRow+j]*grid.basis[index];
                //grid.updates[i] += mat.mat_coef[0+j]*grid.basis[index];
            }
        }
    }
...
// ********************************************************************************************** //
void jacobi2(Grid& grid, Matrix& mat, double& omega){
    int c1, c2, j, index ;
    for (int i = 0; i < grid.n_sup; i++){
        grid.updates[i] = grid.basis[i];
        c1 = i*mat.maxRow;
        c2 = grid.support[i]*mat.maxRow;
        for (j = 0; j < mat.maxRow; j++){
            index = mat.sup_index[c1+j];
            if (index==-1) break;
            if (index!=-2){
                grid.updates[i] += mat.mat_coef[c2+j]*grid.basis[index];
            }
        }
    }
