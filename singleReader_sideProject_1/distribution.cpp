...
Graph& graph, Node& node){
    if(node.number==0){
        ...
        // new stuff //////////////////////////////////////////
        int* temp = new int [l_grid.n_sup];
        // copy cellnumber into temp
        for (int j = 0; j < l_grid.n_sup; j++){
            temp[j] = l_grid.support[j];
        }
        ...
        sortNDremvDup(temp, l_grid.n_sup, cUnique, n_cUnique);
        ...
        //A map that takes in the cell index and returns the index where it is found in mat_coef.
        int* map2 = new int[l_grid.n_sup];

        for (int i = 0; i<l_grid.n_sup; i++){
            int cellNumber = l_grid.support[i];
            map2[i] = map[cellNumber -min];
        }

        l_mat.n_cUnique = n_cUnique;
        l_mat.map = map2;

        double* newCoef = new double[l_mat.maxRow*n_cUnique];
        // make map and new Coefficient matrix.
        for (int i = 0; i < n_cUnique; i++ ){
            for (int j = 0; j < l_mat.maxRow; j++){
                newCoef[i*l_mat.maxRow + j] =
                    g_mat.mat_coef[ l_grid.support[cUnique[i]]*l_mat.maxRow + j];
            }
        }

        l_mat.mat_coef = newCoef;
        ...
        // Second recieving (mat_index in l_mat)
        ...
        // Third recieving (basis in l_grid and mat_coef in l_mat)
