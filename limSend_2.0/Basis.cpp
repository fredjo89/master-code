...
/* ************************************************************************** */
void Basis::makeBasis(Grid& grid, Matrix& mat, double* initB,int n, Options& opt){
  omega = opt.omega;
  tolerance = opt.tolerance;
...
/* ************************************************************************** */
bool Basis::jacobiProduct(bool check_convergence){
  bool done = true;
  for (int i = 0; i<size; i++){
    if(celltypes[i]!=1){
      update[i] = values[i];
  ...
  if (!check_convergence){
    ...
    for (int i =0; i<size; i++){
      if (celltypes[i]==0){
        ...
        if (done==true && fabs(update[i])>tolerance) done = false;
      }
    }
  }
