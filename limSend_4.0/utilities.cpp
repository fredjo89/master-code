...
void checkNormalization(Basis* basisArray, int nBasis, int RANK, int SIZE, int N){
  ...
  for (int i = 0; i<nBasis; i++){
    for (int j = 0; j<basisArray[i].size; j++){
      if (basisArray[i].celltypes[j]!=1) {
        if (abs(recvBuff[basisArray[i].support[j]]-1)>error){
          error = abs(recvBuff[basisArray[i].support[j]]-1);
        }
      }
    }
  }
...
void computeDiscrepancy(Grid& grid, double* GSol,  double* ompSol  ){
  ...
  for (int i = 0; i<grid.n_basis; i++){
    ...
    //if (i%15==0) cout<<endl<<endl;

    double temp = abs(GSol[i] - ompSol[i]);
    //cout<<grid.support[i]<<"\t"<<grid.celltypes[i]<<"\t"<<GSol[i] - ompSol[i]<<endl;
...
/* ************************************************************************** */
void printPoperator(Grid& grid, double* basisArray ){
  int N = grid.N;
  int M = grid.M;
  int n_basis = grid.n_basis;
  int* support = grid.support;
