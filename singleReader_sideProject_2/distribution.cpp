...
void METIS_partition(Graph& graph, Graph& f_graph){
    ...
    int* vSize = new int[f_graph.n_nodes];

    for (int i = 0; i < f_graph.n_nodes; i++){

        vSize[i] = 0;
        for (int j = f_graph.edgeOffsets[i] ; j < f_graph.edgeOffsets[i+1] ; j++){
            vSize[i]+=f_graph.edgeWeights[j];
        }
    ...
    //options[METIS_OPTION_SEED] = MPI_Wtime();
