...
void renormalize(Grid& grid,Matrix& mat, double* basis){
  ...
  double sumval[N]; for (int i = 0; i<N; i++) sumval[i] = 0;
	for(int i = 0; i < n_basis; i++){ sumval[grid.support[i]] += basis[i]; }
	for(int i = 0; i < n_basis; i++){ basis[i] = basis[i]/sumval[grid.support[i]]; }
}
/* ************************************************************************** */
