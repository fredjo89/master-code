...
N = 3*30;                            % Number of cells
n = N/3;                             % Number of cells in each block
M = ceil(N / n);                    % Number of blocks
G = cartGrid([N N]); 
G = computeGeometry(G);
rock = makeRock(G, 1, 1);
rock.perm = logNormLayers(G.cartDims,'sigma', 14, 'sz', [41 9 9 ]);
...
FigHandle = figure('Position', [1200, 800, 800, 800]);
d = reshape(theBasis, G.cartDims); 
...
hold on; 
plotCellData(G,log(rock.perm),'EdgeColor', 'none')
axis equal tight off; 
colormap(jet(32));
TEMP = jet; 
myColor = [0.65 0.65 0.65]
