...
%}
    ...
    if opt.doSolve
        timer = tic();
        fprintf('\n ORIGINAL FUNCTION STATS HERE\n');
        I_original = mex_cppMultiscaleBasis(offsets, support, types, A', I_com,...
                                   opt.tolerance/opt.omega, toIntegerValue(opt.maxiter), ...
                                   opt.omega, toIntegerValue(opt.maxThreads));
        report.t_basis = toc(timer);
        % My function here. 
        
        fprintf('\n MY FUNCTION STATS HERE\n');
        I_mine = my_mex_cppMultiscaleBasis(offsets, support, types, A', I_com,...
                                   opt.tolerance/opt.omega, toIntegerValue(opt.maxiter), ...
                                   opt.omega, toIntegerValue(opt.maxThreads));
        
        Error = max(abs(I_original-I_mine)); 
        Error
        I_com = I_original;
        fprintf('\n'); 
    end
