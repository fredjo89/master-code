...
void makeCoarseGraph(Grid& grid, Matrix& mat, int& SIZE, Graph& graph){
    ...
    // Create connection matrix between nodes
...
void makeFineGraph(Grid& grid, H_data& H, Graph& f_graph){
    ...
    // maxEdge: upper estimate of number of edges
...
void setupNodes(Graph& graph, Node& node, int& RANK){
    ...
    if (RANK==0){
        ...
        MPI_Recv(buffer, 5, MPI_INT, 0, 666, MPI_COMM_WORLD, &status[0] );
        node.number = RANK;
        ...
        MPI_Recv(node.edges, node.n_edges, MPI_INT, 0, 666, MPI_COMM_WORLD, &status[1] );
...
Graph& graph, Node& node){
    if(node.number==0){
        ...
        for (int i = 1; i < graph.n_nodes; i++){
            ...
            for (int j = bNumber_start; j < bNumber_end; j++){
                for (int l=offsets[order[j]]; l<offsets[order[j]+1]; l ++){
                    ...
                    sendBuff1[n_sup + k] = g_grid.celltypes[l];
                    k++;
                }
            }
            k = 2*n_sup;
            ...
            int cell_n = cUnique[k];                                // cellNumber
            for (int j = 0; j < n_sendBuff2; j++){
                ...
                sendBuff2[j] = g_mat.j_index[ cell_n*g_mat.maxRow +col ];
                col++;
            }
            ...
            for (int j = n_sup; j < n_sendBuff3; j++){
                ...
                sendBuff3[j] = g_mat.conn[ cell_n*g_mat.maxRow +col ];
                col++;
            }
        ...
        l_grid.celltypes = new int[ l_grid.n_sup ];
        ...
        for (int j = bNumber_start; j < bNumber_end; j++){
            ...
            for (int l = offsets[ order[j] ]; l < offsets[ order[j]+1 ]; l ++){
                ...
                l_grid.celltypes[k] = g_grid.celltypes[l];
        ...
        l_mat.loc_index = new int[l_mat.maxRow*l_mat.n_sup];
        l_mat.loc_conn = new double[l_mat.maxRow*l_mat.n_sup];
        k = 0;
        int col = 0;
        ...
        for (int j = 0; j < l_grid.n_sup*l_mat.maxRow; j++){
            ...
            l_mat.loc_index[j] = g_mat.j_index[cell_n + col];
            l_mat.loc_conn[j] = g_mat.conn[cell_n + col];
            col++;
        }
        ...
        l_mat.n_sup = node.weight;
        l_mat.loc_index = new int[l_mat.maxRow*l_grid.n_sup];
        l_mat.loc_conn = new double[l_mat.maxRow*l_grid.n_sup];
        ...
        for (int i = l_grid.n_sup; i < 2*l_grid.n_sup; i++){
            l_grid.celltypes[k] = recvBuff1[i];
            k++;
        }
        k = 0;
        ...
        // Second recieving (j_index in l_mat)
        int n_recvBuff2 = l_mat.maxRow*n_cUnique;
        l_mat.j_index = new int[n_recvBuff2];
        MPI_Recv(l_mat.j_index, n_recvBuff2, MPI_INT, 0, 666, MPI_COMM_WORLD, &status[1] );

        // Third recieving (basis in l_grid and conn in l_mat)
        ...
        l_mat.conn = &recvBuff3[l_grid.n_sup];

        // Creating loc_index and loc_conn in l_mat
        int min = cUnique[0];
        ...
        for (int i = 0; i < l_mat.maxRow*l_grid.n_sup; i++){
            ...
            l_mat.loc_index[i] = l_mat.j_index[index + col];
            l_mat.loc_conn[i] = l_mat.conn[index +col];
            col++;
        }

        delete[] recvBuff1;
        delete[] l_mat.j_index;
        delete[] recvBuff3;
        delete[] map;
        delete[] cUnique;
